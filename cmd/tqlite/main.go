// Command tqlite is the command-line interface for tqlite.
package main
...
func main() {
	...
	cli.Run(new(argT), func(ctx *cli.Context) error {
		...
		fmt.Println("Welcome to the tqlite CLI. Enter \".help\" for usage hints.")
		fmt.Printf("Version %s, commit %s, branch %s\n", cmd.Version, cmd.Commit, cmd.Branch)
		fmt.Printf("Connected to tqlited version %s\n", version)
...
func getVersionWithClient(client *http.Client, argv *argT) (string, error) {
	...
	version, ok := resp.Header["X-Tqlite-Version"]
	if !ok || len(version) != 1 {
		return "unknown", nil
	}
